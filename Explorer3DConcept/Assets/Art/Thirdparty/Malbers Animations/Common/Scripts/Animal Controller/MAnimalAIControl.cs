        ...
        public virtual void SetTarget(Transform target, bool Move)
        {
           // Debug.Log("Set target = " + target);
            ...
            if (this.target != null)
            {
               ...
               // Debuging($"<color=yellow>New Target: <B>{target.name}</B></color>");

                StopWaitCoroutine(); //If the Animal was waiting  IMPORTANT
                ...
                if (OMLData.linkType == OffMeshLinkType.LinkTypeManual)              //Means that it has a OffMesh Link component
                {
                    ...
                    if (CurrentOML)
                    {
                        ...
                        if (CurrentOML.CompareTag("Fly"))
                        {
                            ...
                            //Debug.Log("OMMESH FLY");
                            if (debug) Debuging($"<color=white>is On a <b>[OffmeshLink]</b> -> [Fly]</color>");
                            FlyOffMesh(FarTransform);
                        }
                        else if (CurrentOML.CompareTag("Climb"))
                        {
                            if (debug) Debuging($"<color=white>is On a <b>[OffmeshLink]</b> -> [Climb]</color>");
                        ...
                        else if (CurrentOML.area == 2)  //2 is Off mesh Jump
                        {
                            ...
                            if (debug) Debuging($"<color=white>is On a <b>[OffmeshLink]</b> -> [Jump]</color>");
